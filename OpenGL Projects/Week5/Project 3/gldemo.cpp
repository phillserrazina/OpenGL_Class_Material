...
//
// Week 6 - Demo 03 - Retained mode rendering 03 (Interleaved Arrays)
//
...
static GLubyte squareVertexIndices[] = { 0, 1, 2, 3, 4 };
...
static VertexModel interleavedStar[] = {

		{ 0.0f, 0.25f,		255, 0, 0,		0.5f, 1.0f},
...
static VertexModel interleavedSquare[] = {

		{ 0.0f, 0.0f,		255, 0, 0,		0.0f, 0.0f},
		{ 0.5f, 0.0f,		0, 255, 0,		1.0f, 0.0f},
		{ 0.5f, 0.5f,		0, 0, 255,		1.0f, 0.9f},
		{ 0.25f, 0.6f,		125, 125, 0,	0.5f, 1.0f},
		{ 0.0f, 0.5f,		0, 255, 0,		0.0f, 0.9f}
};
...
void drawStarPackedIndexedArray(void);
void drawStarInterleavedArray(void);
void drawSquareInterleavedArray(void);
...
void drawStarPackedIndexedArray(void) {
	...
	glDrawElements(GL_LINE_LOOP, 10, GL_UNSIGNED_BYTE, starVertexIndices);
}


void drawStarInterleavedArray(void) {

	glVertexPointer(2, GL_FLOAT, sizeof(VertexModel), interleavedStar);
	glColorPointer(3, GL_UNSIGNED_BYTE, sizeof(VertexModel), &(interleavedStar[0].r));
	glTexCoordPointer(2, GL_FLOAT, sizeof(VertexModel), &(interleavedStar[0].u));
...
void drawSquareInterleavedArray(void) {
	...
	glVertexPointer(2, GL_FLOAT, sizeof(VertexModel), interleavedSquare);
	glColorPointer(3, GL_UNSIGNED_BYTE, sizeof(VertexModel), &(interleavedSquare[0].r));
	glTexCoordPointer(2, GL_FLOAT, sizeof(VertexModel), &(interleavedSquare[0].u));

	glDrawElements(GL_LINE_LOOP, 5, GL_UNSIGNED_BYTE, squareVertexIndices);
}
